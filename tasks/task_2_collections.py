...
def remove_values_from_list(values: List, values_to_remove: Iterable) -> List:
    spisok = []
    for value in values:
        if value not in set(values_to_remove):
            spisok.append(value)
...
def update_and_merge_dict_using_kwargs(dictionary: Dict, **kwargs) -> Dict:
    slowar = dictionary.copy()
    for key in kwargs:
        if key in slowar:
            if isinstance(slowar[key], list) and isinstance(kwargs[key], list):
                slowar.update({key: slowar[key] + kwargs[key]})
            elif isinstance(slowar[key], list):
                slowar.update({key: slowar[key] + [kwargs[key]]})
            ...
            slowar[key] = kwargs[key]

    return slowar


print(update_and_merge_dict_using_kwargs({'a': [21, 4, 5], 'b': 3, 'c': 4, 'd': 5, 'e': 7}, a=[12, 1, 2], b=6, f=19))
...
def merge_two_dicts(first: Dict, second: Dict) -> Dict:
    ...
    for key in second:
        if key in slowar:
            if isinstance(slowar[key], list) and isinstance(second[key], list):
                slowar.update({key: slowar[key] + second[key]})
            elif isinstance(slowar[key], list):
                slowar.update({key: slowar[key] + [second[key]]})
            elif isinstance(second[key], list):
                slowar.update({key: [slowar[key]] + second[key]})
            else:
                slowar.update({key: [slowar[key], second[key]]})
        else:
            slowar[key] = second[key]
    ...
    for k in second:
        if k in slow:
            if isinstance(slow[k], dict) and isinstance(second[k], dict):
                ...
                if isinstance(slow[k], list):
                    slow.update({k: slow[k] + [second[k]]})
                elif isinstance(second[k], list):
                    slow.update({k: [slow[k]] + second[k]})
                else:
                    slow.update({k: [slow[k], second[k]]})
        else:
...
print(clear_dummy_elements({1: {}, 2: 3, 3: 4, 4: [], 5: None, 6: [], 7: False, 8: 0, 9: 0.0}))
...
def clear_dummy_and_duplicate_elements(dictionary: Dict) -> Dict:
    ...
    for k, v in dictionary.items():
        if v not in (None, '', [], {}) and v not in slow.values():
            slow[k] = v
    return slow
...
def count_dict_elements(dictionary: Dict) -> Dict:
    return {'numbers': sum(1 for v in dictionary.values() if isinstance(v, numbers.Number) and -10 <= v <= 25)}
